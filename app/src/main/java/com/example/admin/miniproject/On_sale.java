...
public class On_sale extends Fragment {

    private RecyclerView recyclerView;
    private Sale_Adapter adapter;
    private List<Artwork> albumList,albumList2;
    private DatabaseReference databaseReference,databaseReference2;
    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        View rootView = inflater.inflate(R.layout.on_sale, container, false);
        recyclerView = (RecyclerView) rootView.findViewById(R.id.recycler_view_onsale);
    ...
    private void prepareAlbums() {
        ...
        if(Filter_Popup.checked.isEmpty() && Price_Filter.price_check.isEmpty()) {
            databaseReference.addValueEventListener(new ValueEventListener() {
                ...
                public void onDataChange(DataSnapshot dataSnapshot) {
                    ...
                    for (DataSnapshot paintshot : dataSnapshot.getChildren()) {
                        if (paintshot.getValue(Artwork.class).getCategory() == 0) {
                            Artwork p = paintshot.getValue(Artwork.class);
                            albumList.add(p);
                        }
                    }
                    adapter = new Sale_Adapter(getActivity().getApplicationContext(), albumList);
            ...
            if (Price_Filter.price_check.isEmpty()){
                adapter = new Sale_Adapter(getActivity().getApplicationContext(), albumList2);
                            ...
                            for (DataSnapshot paintshot : dataSnapshot.getChildren()) {
                                ...
                                FirebaseDatabase.getInstance().getReference("All_Paintings/"+obj).addValueEventListener(new ValueEventListener() {
                                    ...
                                    public void onDataChange(DataSnapshot dataSnapshot) {
                                        ...
                                        if(dataSnapshot.getValue(Artwork.class).getCategory()==0) {
